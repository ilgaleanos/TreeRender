...
class Home extends Layout {
        constructor(props) {
                ...
                this.mData = {
                        items:[
                                {id:1, active:false, props:[1,2,3]},
                                {id:2, active:true, props:[11,12,13]},
                                {id:3, active:false, props:[21,22,23]},
                                {id:4, active:true, props:[31,32,33]},
                        ]
                };
                ...
                setTimeout(()=>{
                        this.mData = {
                                items:[
                                        {id:10, active:false, props:[1,2,3]},
                                        {id:20, active:false, props:[1,2,3]},
                                        {id:30, active:true, props:[1,2,3]},
                                        {id:40, active:true, props:[1,2,3]},
                                ]
                        };
                        mPalette.render({
                                '.Table': {
        ...
        templateHome() {
                return(
                        <div>
                                ...
                                <div>
                                        {
                                                this.store.arr.map((el)=>{return(
                                                        <div>{ this.store.text }</div>
                                                )})
                                        }
                                </div>
                                <table class="Table" onclick={(event)=>{console.log(event.target.getAttribute("data-text")); return false;} }><tbody>
                                        {
                                                this.mData.items.map((item)=>{return(
                                                        <tr class={ "TableRow "+ (item.active ? 'active' : '') } data-id={ item.id }>
                                                                <td class="TableCell" data-text={"#"+ item.id } >{ "#" + item.id }</td>
                                                                {
                                                                        item.props.map((prop)=>{return(
                                ...
                                <div>
                                        <input onkeyup={ this.changeInput.bind(this) }  value={ this.store.text }/>
                                        {
                                                Part(this.store.arr)
                                        }
                                </div>
        ...
        render() {
                autorun(()=>{
                        RenderDiff(
                                this.containerHome, this.templateHome(),
                                "Home", this.store
                        );
                });
        }
